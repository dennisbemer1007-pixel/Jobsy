...
- **Progressive disclosure:** Werkgevers zien kandidaat-PII (naam, e-mail, telefoon, adres, stad, work-permit, CV-snapshots) pas na acceptatie (`Accepted` / `EmployerContacting` / `Hired`). Pre-accept wél zichtbaar voor screening: motivatie, match-%, crow-flies afstand (km, zonder adres), leeftijd en beschikbaarheidssamenvatting. Het automatisch gegenereerde **Lobsy-CV (PDF)** en het **geüploade kandidaat-CV** volgen dezelfde vrijgave: download voor werkgevers alleen post-Accept; kandidaat mag eigen bestanden altijd. Autorisatie op de PDF/upload-endpoints (niet alleen UI). Spec: `docs/FUNCTIONELE_SPECIFICATIES_CV_PREVIEW_MODERATIE.md`. Recensies (derde-partij contact) staan in `CandidateReferences`, gaan mee in export/anonymize, en worden niet aan werkgevers getoond tot Accept (alleen de telling is intern).

...
- **Session inactivity:** Admin-configurable `SessionInactivityTimeoutMinutes` (default 30, clamp 5–480) on platform features. Web `SessionInactivityMiddleware` + browser idle-timer read it dynamically; idle sessions are signed out and redirected to `/login?error=session-expired` (critical form drafts saved to `sessionStorage` where possible).

...
| Intermediair | Lege clients / API-fout. | Empty/error. |
...
| Intermediair | Invite als BranchManager/Admin. | Niet mogelijk in UI. |
...
| Intermediair | Nieuwe vacature: zonder KVK/opdrachtgever opslaan. | Geblokkeerd; KVK verplicht. |
| Intermediair | Create: KVK **Zoek vestigingen** + kies vestiging / bekende opdrachtgever. | Company gezet. |
| Intermediair | Create: KVK onbekend/storing. | Fout; niet publiceren tot Verified. |
...
| Intermediair | Categorie-dropdown. | Vergrendeld Uitzendbureau. |
...
| Intermediair | Tokens allocate naar ‘vestigingen’. | Niet (geen EM-allocate). |
| Intermediair | Company page `/employer/company`. | Contact/billing/API zoals employer. |
| Intermediair | Open `/employer/organization`, `/employer/users` (EM-users), `/employer/branches` EM-invites. | 403 of geen EM-acties. |
| Intermediair | Open `/admin/*`. | 403. |
| Intermediair | CSV/API import zonder KVK. | Rejected rows (spec). |
...
| Intermediair | HasCandidateApplications nav. | Read-only eigen sollicitaties. |

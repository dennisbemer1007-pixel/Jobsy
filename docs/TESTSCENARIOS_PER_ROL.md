...
| Gast | Klik op `/privacy` de link **Mijn gegevens** (`/privacy/data`). | Redirect naar login (Authorize). Na login zou export beschikbaar zijn. |
...
| Gast | Cookie-banner: **Alleen noodzakelijk**. | Banner verdwijnt. Geen analytics-POST. Keuze onthouden (localStorage + cookie). |
...
| Gast | Login-modal: klik backdrop of **×**. | Terug naar banenkaart `/` (niet naar een protected returnUrl). |
| Gast | Login-modal: **Terug naar de kaart**. | Zelfde: `/`. |
| Gast | Login: **Registreer via KVK**. | `/register` wizard stap KVK. |
| Gast | Login: leeg e-mail/wachtwoord submit. | HTML5 required blokkeert of serverfout; geen sessie. |
| Gast | Login: ongeldig wachtwoord (`niet@bestaat.nl` / `fout`). | Foutmelding (geen stacktrace/PII); blijft op login. |
| Gast | Login: Microsoft-knop terwijl Entra niet geconfigureerd is. | Knop disabled + tooltip ‘niet geconfigureerd’. |
| Gast | Login: Google-knop terwijl Google niet geconfigureerd is. | Knop disabled + tooltip. |
| Gast | Login: Microsoft/Google wél geconfigureerd, daarna IdP-annulering. | Terug op login met foutcode; geen half-sessie. |
| Gast | Login: Google-account met `email_verified=false`. | Weigering; geen accountkoppeling. |
| Gast | Open `/login?error=session-expired`. | Melding dat de sessie is verlopen; kan opnieuw inloggen. |
| Gast | Open `/login?returnUrl=https://evil.example`. | returnUrl wordt genegeerd/gesaneerd; geen open redirect. |
| Gast | Open `/login?returnUrl=/admin/users`. | Na geslaagde kandidaat-login géén toegang tot admin (Access Denied of home); local returnUrl alleen als toegestaan. |
| Gast | Open `/access-denied`. | Tekst over geen toegang; links **Naar home** en **Inloggen**. |
| Gast | Access-denied: **Naar home** zonder login. | Challenge naar login of `/` volgens FallbackPolicy. |
| Gast | Open `/Error`. | Generieke foutpagina, eventueel request-referentie, **geen** stacktrace; link **Naar de banenkaart**. |
...
| Gast | Banenkaart: adres wissen (**Clear**). | Origin weg; empty-state zonder reistijd of fallback-weergave. |
| Gast | Banenkaart: **Gebruik mijn locatie** + toestemming. | GPS-origin; lijst herberekend. |
| Gast | Banenkaart: locatie **weigeren**. | Foutmelding locatie; geen crash; handmatig adres blijft mogelijk. |
| Gast | Banenkaart: locatie unavailable (timeout). | Foutmelding; filters blijven staan. |
| Gast | Banenkaart: open **Filters**. | Filter sheet/bar met keyword, type, categorie, leeftijd, loon, reistijd, uren, vervoer, radius. |
| Gast | Filters: **Annuleren** zonder toepassen. | Sheet dicht; oude filters blijven. |
...
| Gast | Filters: zonder leeftijd lonen bekijken op kaart/lijst. | Geen loonbedragen (privacy). |
...
| Gast | Filters: min-loon > max-loon. | Validatie of lege set; geen 500. |
...
| Gast | Banenkaart: **Bewaar** (zoekopdracht). | Niet zichtbaar voor gasten (alleen na inloggen). |
...
| Gast | Vacaturedetail: vervoer wijzigen. | Reistijd herberekend of melding ‘locatie nodig’. |
| Gast | Vacaturedetail: **Route** zonder locatietoestemming. | Fout of prompt; geen crash. |
| Gast | Vacaturedetail: **Route** mét locatie. | Externe maps-directions. |
| Gast | Vacaturedetail: **Street View**. | Externe Street View of foutmelding. |
| Gast | Vacaturedetail: **Video kijken** indien URL. | Externe video; ontbreekt: knop niet getoond. |
...
| Gast | Vacaturedetail: onbekende GUID. | Niet-gevonden/fout; geen PII; geen 500-HTML met exception. |
...
| Gast | Open onbekend kvk-pad. | Not-found empty-state of 404, geen 500. |
...
| Gast | Open `/vestiging/{onbekende-guid}`. | Fout/redirect zonder 500. |
...
| Gast | Open `/partner/AM-XXXXXX` of ongeldige code. | Tracking genegeerd; generieke partnerpagina (AM hoort bij `/werven`). |
| Gast | Open `/werven/{ongeldige-code}`. | Flash + redirect `/login` **zonder** ambassadeur-cookie. |
...
| Gast | Feedback: leeg versturen. | Validatie of disabled submit. |
...
| Gast | Register: vestiging die al in gebruik is. | Geen Toevoegen; **claim**-pad aangeboden. |
...
| Gast | Register: KVK-storing (unavailable). | Handmatig vestiging + **Doorgaan (KVK in afwachting)**; geen Intermediair-rol; publiceren/tokens later geblokkeerd tot Verified. |
| Gast | Register KVK-pending: **Terug**. | Terug naar KVK-stap. |
| Gast | Register contact: wachtwoord te kort / mismatch. | Validatiefout; niet verzonden. |
| Gast | Register contact: voorwaarden/privacy niet aangevinkt. | Submit disabled of fout. |
...
| Gast | Open `/swagger` op Web-host. | Niet de API-swagger; of 404. API-swagger buiten Development uit tenzij flag. |
| Gast | Open `/tokens/checkout-stub` of payout-stubs. | Login of 403; geen anonieme betaling. |
...
| Rol | Testscenario | Verwacht resultaat |
|-----|--------------|--------------------|
| Kandidaat | Login met demo-account. | `/home` kandidaat-dashboard **Mijn Lobsy Kompas** (tabbladen **Mijn profiel** / **Mijn competenties** / **Mijn beroepen**) plus KPI’s sollicitaties/likes/shares/reacties. |
| Kandidaat | Home: Kompas-tabbladen Mijn profiel / Mijn competenties / Mijn beroepen. | Drie tabs; profiel=beschikbaarheid/reistijd/vervoer/rijbewijs; competenties=Quick-Scan 25 vs diepte-analyse 150 + werkstijl; beroepen=Super-match/Sterke keus/Handige verbreding + Wat betekent dit voor jou? + PDF. |
...
| Admin | Settings-subnav: klik **elk** item (Settings, CNAMEs, Company, About, Marketing flyer, Masterdata, Vacaturecategorieën, Exclusivity, Integraties, Mail test, API keys, Notifications, Users, Logging, Feedback, Wages). | Elke module laadt; active state. Pills wrappen zodat rechter items klikbaar blijven. |
